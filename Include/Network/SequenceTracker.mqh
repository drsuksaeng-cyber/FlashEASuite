...
CSequenceTracker::CSequenceTracker(string storage_file = "policy_sequences.csv")
{
    ...
    Print("[OK] SequenceTracker initialized");
...
bool CSequenceTracker::ValidateSequence(const string symbol, long sequence)
{
    ...
    if(last_seq == 0)
    {
        ...
        {
            Print("[ERROR] SECURITY ALERT: Invalid first sequence!");
            Print("   Symbol: ", symbol);
            Print("   Expected: 1");
    ...
    {
        Print("[ERROR] SECURITY ALERT: Out-of-order sequence!");
        Print("   Symbol: ", symbol);
...
void CSequenceTracker::UpdateSequence(const string symbol, long sequence)
{
    ...
    for(int i = 0; i < m_sequence_count; i++)
    {
        if(m_sequences[i].symbol == symbol)
        {
            ...
            SaveToFile();
            Print("[OK] Sequence updated: ", symbol, " → ", sequence);
            return;
        }
    }
    ...
    if(m_sequence_count < 50)
    {
        ...
        m_sequence_count++;
        SaveToFile();
        Print("[OK] New symbol tracked: ", symbol, " (seq: ", sequence, ")");
    }
    else
    {
        Print("[WARN] WARNING: Symbol limit reached (50)");
...
void CSequenceTracker::SaveToFile()
{
    ...
    {
        Print("[WARN] Failed to save sequences: ", GetLastError());
        return;
    }
...
{
    Print("[STATS] Sequence Tracker Statistics:");
    Print("   Tracked symbols: ", m_sequence_count);
...
void CSequenceTracker::ResetSymbol(const string symbol)
{
    for(int i = 0; i < m_sequence_count; i++)
    {
        if(m_sequences[i].symbol == symbol)
        {
            ...
            m_sequence_count--;
            SaveToFile();
            Print("[WARN] Reset sequence for: ", symbol);
            return;
        }
    }
}
...
void CSequenceTracker::ResetAll()
{
    ...
    Print("[WARN] All sequences reset");
}
