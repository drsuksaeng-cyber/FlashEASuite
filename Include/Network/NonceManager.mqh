...
CNonceManager::CNonceManager(int cleanup_interval = 3600)
{
    ...
    // Initialize string array manually (ArrayInitialize not supported for strings)
    for(int i=0; i<1000; i++) m_used_nonces[i] = "";
    ArrayInitialize(m_nonce_timestamps, 0);
    
    Print("[OK] NonceManager initialized");
...
bool CNonceManager::IsNonceUsed(const string nonce)
{
    ...
    for(int i = 0; i < m_nonce_count; i++)
    {
        ...
        {
            Print("[WARN] SECURITY ALERT: Nonce already used!");
...
bool CNonceManager::StoreNonce(const string nonce)
{
    ...
    if(m_nonce_count >= 1000)
    {
        ...
        if(m_nonce_count >= 1000)
        {
            Print("[WARN] WARNING: Nonce storage full!");
...
{
    Print("[STATS] Nonce Manager Statistics:");
