//+------------------------------------------------------------------+
//| MM01_FixedConservative.mqh                                       |
//| FlashEASuite V2 V6 — MM01: Fixed Fractional Conservative         |
//+------------------------------------------------------------------+
//| FORMULA:                                                         |
//|   RiskAmount = Balance × 1% (or Equity if equity_type=1)         |
//|   Lot = RiskAmount / (SL_distance × tick_value/tick_size)        |
//|                                                                  |
//| PARAMS (from CONFIG_PUSH):                                       |
//|   MM01_RISK_PCT    default=1.0%  range[0.5, 3.0]                 |
//|   MM01_EQUITY_TYPE default=1 (1=Equity, 0=Balance)              |
//|   MM01_MIN_LOT     default=0.01                                  |
//+------------------------------------------------------------------+
#property copyright "Dr. Suksaeng Kukanok"
#property version   "6.00"
#property strict

#ifndef MM01_FIXED_CONSERVATIVE_MQH
#define MM01_FIXED_CONSERVATIVE_MQH

#include "IMoneyManager.mqh"

class CMM01_FixedConservative : public IMoneyManager
{
private:
    double  m_risk_pct;         // Risk percent per trade (default 1.0)
    int     m_equity_type;      // 0=Balance, 1=Equity
    double  m_min_lot;          // Minimum lot override

public:
    //--- Default constructor
    CMM01_FixedConservative()
    {
        m_risk_pct    = 1.0;
        m_equity_type = 1;          // Use Equity by default
        m_min_lot     = 0.01;
    }
    ...
    virtual void Setup(string symbol, int history_window = 50) override
    {
        ...
        // m_min_lot will be updated from broker info
        m_min_lot = MathMax(0.01, SymbolInfoDouble(symbol, SYMBOL_VOLUME_MIN));
    }
    
    //+------------------------------------------------------------------+
    //| SetParams: Apply parameters from CONFIG_PUSH or manual input     |
    //+------------------------------------------------------------------+
    ...
    //+------------------------------------------------------------------+
    //| CalculateLot: Fixed Fractional 1% risk                          |
    //+------------------------------------------------------------------+
                                 ...
                                 double stopLoss, string symbol) override
    {
        ...
        // Calculate lot: risk_amount / value_of_sl_distance_per_lot
    ...
    virtual string GetName() override { return "MM01_FixedConservative"; }
    virtual int    GetID()   override { return (int)MM_ID_FIXED_CONSERVATIVE; }
    
    virtual string GetDiagnostic() override
    {
        string base_str = (m_equity_type == 1) ? "Equity" : "Balance";
        return StringFormat("[MM01] Risk:%.1f%% Base:%s Multiplier:%.2f",
            m_risk_pct, base_str, m_risk_multiplier);
    }
};

#endif // MM01_FIXED_CONSERVATIVE_MQH
