...
public:
    ...
    //+------------------------------------------------------------------+
    //| Determine Market State                                           |
    //| ADJUSTED: More lenient ADX thresholds for Grid Strategy         |
    //+------------------------------------------------------------------+
    void DetermineMarketState()
    {
        ...
        // ADJUSTED THRESHOLDS:
        // - Ranging: ADX < 25 (was 20) - More room for ranging
        // - Weak Trend: ADX 25-40 (was 20-30) - Grid can work here
        // - Strong Trend: ADX >= 40 (was 30) - Only reject very strong trends
        
        if(adx_value < 25)  // ⬆ Increased from 20
        {
            // Ranging market - Grid works well
            m_is_ranging = true;
            m_is_trending = false;
        ...
        else if(adx_value >= 25 && adx_value < 40)  // ⬆ Changed from 20-30 to 25-40
        {
            // Weak trend - Grid can still work here
            m_is_trending = true;
            m_is_ranging = false;
            m_current_market_state = MARKET_STATE_TRENDING_WEAK;
        }
        else  // ADX >= 40 (⬆ Changed from >= 30)
        {
            // Strong trend - Reject Grid, use Spike instead
            m_is_trending = true;
            m_is_ranging = false;
