...
class SpikePatterns:
    ...
    def generate_spike_entry(
        ...
        for i in range(num_ticks):
            ...
            # FIXED: Send as array (list) to match FeederEA protocol
            # Format: [Type, SeqID, Timestamp, Symbol, Bid, Ask, Flags]
            tick = [
                1,                          # [0] msg_type: 1 = TICK_DATA
                i + 1,                      # [1] seq_id
                timestamp + (i * 50),       # [2] timestamp (milliseconds)
                symbol,                     # [3] symbol
                current_price,              # [4] bid
                current_price + spread,     # [5] ask
                6                           # [6] flags (TICK_FLAG_BID | TICK_FLAG_ASK)
            ]
