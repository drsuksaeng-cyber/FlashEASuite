...
void CheckForPolicies()
{
   ...
   // CRITICAL FIX: Strip broker suffix (.tp, .m, etc.) before comparison
   string chart_symbol = _Symbol;
   string policy_symbol = policy.symbol;
   
   // Remove common broker suffixes
   StringReplace(chart_symbol, ".tp", "");
   StringReplace(chart_symbol, ".m", "");
   StringReplace(chart_symbol, ".i", "");
   StringReplace(policy_symbol, ".tp", "");
   StringReplace(policy_symbol, ".m", "");
   StringReplace(policy_symbol, ".i", "");
   
   if(policy_symbol != chart_symbol)
   {
      Print("⚠️  Policy symbol mismatch: ", policy.symbol, " (cleaned: ", policy_symbol, ") != ", _Symbol, " (cleaned: ", chart_symbol, ")");
      return;
   }
   
   Print("✅ Symbol match: ", policy.symbol, " → ", chart_symbol);
