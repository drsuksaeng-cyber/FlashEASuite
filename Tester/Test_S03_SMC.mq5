...
//|  2. เปิด MetaTrader 5 → ไปที่ chart XAUUSD.tp H1                    |
...
//--- Script inputs
input string  Test_Symbol    = "XAUUSD.tp";   // Symbol to test on
...
void Test_OrderBlockDetector(string symbol, ENUM_TIMEFRAMES tf)
{
    ...
    COrderBlockDetector ob_det;
    ob_det.Setup(symbol, tf, 50, 1.5, 0.5);
    ob_det.Scan();
...
void Test_FVGDetector(string symbol, ENUM_TIMEFRAMES tf)
{
    ...
    CFVGDetector fvg_det;
    fvg_det.Setup(symbol, tf, 50, 0.5);
    fvg_det.Scan();
...
void Test_BOSDetector(string symbol, ENUM_TIMEFRAMES tf)
{
    ...
    CBOSDetector bos_det;
    bos_det.Setup(symbol, tf, 5);
    bos_det.Detect(50);
    ...
    if(has_bos)
    {
        PrintFormat("     Direction: %s | BreakLevel: %.2f | Time: %s",
            last.direction == BOS_DIR_BULLISH ? "BULLISH" : "BEARISH",
            last.break_level,
