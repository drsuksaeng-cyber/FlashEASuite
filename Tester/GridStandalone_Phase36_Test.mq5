...
//|                                FlashEASuite V2 - Phase 3.5 + 3.6|
//|              Standalone Test EA for ATR + Swap Protection        |
//+------------------------------------------------------------------+
#property copyright "Dr. Suksaeng Kukanok"
#property link      "FlashEASuite V2"
#property version   "2.00"
#property description "ทดสอบ Grid Strategy Phase 3.5 + 3.6 โดยไม่ต้องรอ Python Brain"
#property strict
...
input group "=== Swap Filter Settings (Phase 3.6) ==="
input bool     EnableSwapFilter = true;            // Enable Swap Filter
...
{
   Print("╔═══════════════════════════════════════════════════════════╗");
   Print("║  Grid Strategy Phase 3.5 + 3.6 - Standalone Test EA     ║");
   Print("╚═══════════════════════════════════════════════════════════╝");
   ...
   //--- Configure Swap Filter (Phase 3.6)
   grid_strategy.SetSwapFilterEnabled(EnableSwapFilter);
   ...
   Print("   Swap Filter: ", EnableSwapFilter ? "Enabled ✅" : "Disabled");
   Print("   Trading Enabled: ", EnableTrading ? "YES ⚠️" : "NO (Safe)");
   
   // Show current swap values
   double swap_long = SymbolInfoDouble(_Symbol, SYMBOL_SWAP_LONG);
   double swap_short = SymbolInfoDouble(_Symbol, SYMBOL_SWAP_SHORT);
   Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
   Print("💰 Current Swap Values:");
   Print("   Long (Buy): ", DoubleToString(swap_long, 2), swap_long > 0 ? " ✅" : " ❌");
   Print("   Short (Sell): ", DoubleToString(swap_short, 2), swap_short > 0 ? " ✅" : " ❌");
   Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
   Print("🎯 Mock Policy:");
...
void RunGridTest()
{
   ...
   if(score > 0)
   {
      ...
      if(VerboseLogging)
      {
         ...
         Print("   • Swap Filter Check: ", EnableSwapFilter ? "✅" : "⚠️ Disabled");
   ...
   else
   {
      ...
      if(VerboseLogging)
      {
         ...
         Print("   • Swap Filter: ", !EnableSwapFilter ? "⚠️ Disabled" : (CheckSwapFilterDiag() ? "✅ PASS" : "❌ FAIL (BLOCKED HERE!)"));
      ...
      Print("   🛡️ Phase 3.5 + 3.6 Protection working correctly!");
   }
...
//+------------------------------------------------------------------+
//| Check Swap Filter (for diagnostic)                               |
//+------------------------------------------------------------------+
bool CheckSwapFilterDiag()
{
   double swap_long = SymbolInfoDouble(_Symbol, SYMBOL_SWAP_LONG);
   double swap_short = SymbolInfoDouble(_Symbol, SYMBOL_SWAP_SHORT);
   
   bool is_buy = (MockGridDirection == 1);
   bool is_sell = (MockGridDirection == 2);
   
   if(VerboseLogging)
   {
      Print("   Swap Details:");
      Print("     Long: ", DoubleToString(swap_long, 2), swap_long > 0 ? " (POSITIVE ✅)" : " (NEGATIVE ❌)");
      Print("     Short: ", DoubleToString(swap_short, 2), swap_short > 0 ? " (POSITIVE ✅)" : " (NEGATIVE ❌)");
      Print("     Direction: ", is_buy ? "BUY" : is_sell ? "SELL" : "NONE");
   }
